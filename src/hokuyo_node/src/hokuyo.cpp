...
hokuyo::Laser::serviceScan(hokuyo::LaserScan& scan, int timeout) {
 ...
 /*   // printf("Scan took %lli.\n", -scan.system_time_stamp + timeHelper() + offset_);
    ROS_INFO("The ranges vector before: ");
    for (float foo : scan.ranges) {
        ROS_INFO("%f ", foo);
    }
    ROS_INFO("\n");

    ROS_INFO("The intensities vector before: ");
    for (float foo : scan.intensities) {
        ROS_INFO("%f ", foo);
    }
    ROS_INFO("\n");
*/
    ...
    /*ROS_INFO("The ranges vector after: ");
    for (float foo : scan.ranges) {
        ROS_INFO("%f ", foo);
    }
    ROS_INFO("\n");

    ROS_INFO("The intensities vector after: ");
    for (float foo : scan.intensities) {
        ROS_INFO("%f ", foo);
    }
    ROS_INFO("\n");*/
